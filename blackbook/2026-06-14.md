...
development. It maps the relationships between the rule files, reference docs, and feature workspaces — and explains how
data flows between them during feature development.
...
(`features/*`) as inputs — they reference feature elements (data-flow diagrams, mock requests, scripts) to inform the
...
**Key:** The `## Feature Inputs` section explicitly references feature workspace elements that inform the plan. A plan
should never duplicate feature workspace content — it should reference it.
...
plans, but the full context including diagrams, mock requests, and helper scripts. Each feature workspace is a complete
